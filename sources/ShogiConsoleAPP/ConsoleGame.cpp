...
namespace APP {
    ...
    void ConsoleGame::start() {
        ...
        isRun = true;
        while (isRun) {
            APP::Command *command = inputCommand();
            if (command != nullptr)
                try {
                    command->execute();
                }
    ...
    APP::Command *ConsoleGame::inputCommand() {
        std::string str;
        std::cin >> str;
        int tmp;
        try {
...
//        }
            case 2: {
                ...
                if (h1 >= 1 && h1 <= 9 && v1 >= 1 && v1 <= 9 && h2 >= 1 && h2 <= 9 && v2 >= 1 && v2 <= 9) {
                    return new APP::Move(game, h1, v1, h2, v2);
                }
                else {
                    std::cout << "Bad arguments of command" << std::endl;
                    return nullptr;
            ...
            case 3: {
                ...
                if (h >= 1 && h <= 9 && v >= 1 && v <= 9) {
                    return new APP::Drop(game, pieceType, h, v);
                }
                else {
                    std::cout << "Bad arguments of command" << std::endl;
                    return nullptr;
            ...
            case 4: {
                return new APP::Exit(this);
            }
            case 5: {
                return new APP::ShowCapturedPieces(this);
            }
            case 6: {
                return new APP::Undo(game);
            }
//        case 7:
//        {
...
//todo refactor
    void ConsoleGame::printListOfCapturedPieces(Player player) {
        ...
        std::cout << "Pawn - " <<
        std::count_if(pieces.begin(), pieces.end(), std::bind1st(std::mem_fun(&Piece::equals), &tmp)) << std::endl;
        ...
        std::cout << "Silver General - " <<
        std::count_if(pieces.begin(), pieces.end(), std::bind1st(std::mem_fun(&Piece::equals), &tmp)) << std::endl;
        tmp = Piece(GoldGeneral, player);
        std::cout << "Gold General - " <<
        std::count_if(pieces.begin(), pieces.end(), std::bind1st(std::mem_fun(&Piece::equals), &tmp)) << std::endl;
    }
